<template>
  <div class="mb-4 sm:mb-6">
    <!-- 移动端下拉选择器 -->
    <div class="block rounded-xl bg-white/10 p-2 dark:bg-gray-800/20 sm:hidden">
      <select
    ...
    <!-- 桌面端标签栏 -->
    <div class="hidden flex-wrap gap-2 rounded-2xl bg-white/10 p-2 dark:bg-gray-800/20 sm:flex">
      <button
        v-for="tab in tabs"
        :key="tab.key"
        :class="[
      ...
      >
        <component :is="tab.icon" class="mr-1 sm:mr-2" :size="16" />
...
import { PhGauge, PhKey, PhUserCircle, PhUsers, PhGraduationCap, PhGear } from '@phosphor-icons/vue'
...
const tabs = computed(() => {
  const baseTabs = [
    { key: 'dashboard', name: '仪表板', shortName: '仪表板', icon: PhGauge },
    { key: 'apiKeys', name: 'API Keys', shortName: 'API', icon: PhKey },
    { key: 'accounts', name: '账户管理', shortName: '账户', icon: PhUserCircle }
  ]
  ...
  baseTabs.push(
    { key: 'tutorial', name: '使用教程', shortName: '教程', icon: PhGraduationCap },
    { key: 'settings', name: '系统设置', shortName: '设置', icon: PhGear }
  )
